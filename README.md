...
> This section is a condensed quick reference. (Full TUI key/click interactions: see the guides above.)
...
> The authoritative module-by-module reference — source tree, every tool, TUI layer, config field,
> and command — lives in dedicated docs:
> [`Doc/architecture.md`](Doc/architecture.md) (English) · [`Doc/架构.md`](Doc/架构.md) (中文).

The core loop is: stream the LLM → parallel tool calls (each passing the Safety gate) → feed results
back as `role=tool`, guarded against dead loops by `maxSteps` and 8000-char result truncation.
