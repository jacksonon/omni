...
"Omni Web" is a standalone desktop app that bundles the web backend (Electron's own Node runtime) — download the artifact for your platform from the **GitHub Releases** page (every `v*` tag builds them automatically):

| Platform | Artifact |
|---|---|
| macOS (Apple Silicon) | `Omni.Web-<version>-arm64-mac.zip` — unzip, drag `Omni Web.app` into Applications |
| macOS (Intel) | `Omni.Web-<version>-mac.zip` — unzip, drag `Omni Web.app` into Applications |
| Windows | `Omni.Web.Setup.<version>.exe` (installer) or `Omni.Web.exe` (portable) |
| Linux | `Omni.Web-<version>.AppImage` — `chmod +x` and run |
