...
**GitHub Actions 自动发布**（`.github/workflows/release.yml`）：推送任意 `v*` tag → 自动 `npm ci` + `npm run build`（dist/omni.cjs）+ `npm run compile`（release/omni，linux-x64）+ `npm pack`（tgz）→ 创建 GitHub Release 并附上 `omni-<版本>.tgz` 与 `omni-linux-x64` 两个产物。**关键坑**：bun 静态解析 `@opentui/core` 的 chunk-bun 按 libc 分支的 import，linux 下需同时提供 gnu+musl 原生包，否则 `npm run compile` 报 `Could not resolve @opentui/core-linux-x64-musl`——workflow 在 compile 前用 `npm i --force --no-save @opentui/core-linux-x64-musl@<版本>` 补装（npm 默认拒绝跨 libc 安装，需 `--force` 绕过；`--no-save` 不污染 package.json/lock）。
