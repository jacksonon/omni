...
> - `computeRows` 尾部窗口裁剪 + **滚动**：高度 - 根框固定(4) - 输入框+状态栏(4 或 1)，超出的部分默认自动跟随最新；`scrollTop` 非 null 时回看历史（内容窗 cap-1 行 + 底部 dim 提示行「↑ 已上滚 N 行 · 共 M 行」）；`scrollIntent`（按键发出的一次性指令）在 computeRows 内消费，滚动数学集中一处；每行按显示列数截断（CJK 全角算 2 列，超长加省略号）并设置 `wrapMode:'none'`——否则 TextBuffer 默认 word 换行会把实际渲染行数撑过预算，yoga 把状态栏/输入框挤出视口；视口 <8 行时隐藏状态栏保证输入框完整；
