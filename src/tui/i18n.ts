...
const ZH: Record<string, string> = {
  ...
  'help.commands': '/settings 设置（状态行 / 语言 / 主题 / token 统计 / 诊断 / 帮助 / 模型快照） · /permission 安全权限（只读/请求批准/帮我批准/完全访问） · /thinking 思考展开/折叠 · /plan 计划模式（只读调研） · /undo 撤销本次会话的文件修改 · /rewind 会话检查点（三模式：仅代码/仅对话/双向） · /init [--global] 生成项目/全局记忆 · /agents 子代理配置 · /orchestrate 并行编排（fan-out+汇总+对抗审查） · /goal 目标机制（自动推导验收标准并循环直至达标） · /context 调整上下文窗口（256/400/512/750/1000K · 默认） · /status 会话状态（含上下文用量） · /exit 退出 · /clear 清空上下文 · /new 新建会话（回到初始状态）',
  ...
  'menu.rewind.title': '会话检查点（三模式回滚：仅代码/仅对话/双向）',
...
const EN: Record<string, string> = {
  ...
  'help.commands': '/settings settings (statusline / language / theme / token stats / diagnostics / help / model snapshot) · /permission security (read/safe/ask/full) · /thinking expand/collapse thinking · /plan plan mode (read-only) · /undo undo file changes · /rewind session checkpoints (code/chat/both) · /init [--global] generate AGENTS.md · /agents subagent config · /orchestrate parallel pipeline (fan-out+combine+review) · /goal goal mechanism (derive criteria and loop until met) · /context set context window (256/400/512/750/1000K · default) · /status session status (incl. context usage) · /exit quit · /clear clear context · /new new session (reset to initial state)',
  ...
  'menu.rewind.title': 'Session checkpoints (code/chat/both)',
